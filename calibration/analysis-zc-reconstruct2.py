...
def model(angular_position, angular_displacement):
    coefficient=1.0
    phase_current_displacement = deg_to_rad(120)
...
params, cov = curve_fit(model, xdata=partial_angle_data, ydata=data_to_fit, p0=[0], sigma=sigmaaaa, maxfev=max_iter)
...
fitted_data = model(angle_data, angular_displacement)
...
plot_title = 'Fit parameters:\n angular_disp=%.2f±%.1f phase_current_disp=120' % (rad_to_deg(angular_displacement), rad_to_deg(errors[0]))
